...
'''
# DHS Images
image_location = '../data/classify_images/' #'../roofs/images/DHS_locations/'
image_csv = '../roofs/csv/DHS_images.csv'
out_image_csv = '../roofs/csv/DHS_images_results.csv'

#Evaluate roof ratios
image_data = pd.read_csv(image_csv)
data_bins = hm.bin_data(image_data, image_location)
ratio_models = load_models('../models/', 'ratio_model_')
ratio_df = apply_models(ratio_models, data_bins, image_location, 'roof_ratio')
ratio_df.set_index('image', inplace=True)
ratio_df.to_csv(out_image_csv)


#Evaluate roof counts
image_data = pd.read_csv(out_image_csv)
data_bins = hm.bin_data(image_data, image_location)
count_models = load_models('../models/', 'count_model_')
count_df = apply_models(count_models, data_bins, image_location, 'roof_counts')
count_df.set_index('image', inplace=True)
count_df.to_csv(out_image_csv)
...
iron_df = apply_models(iron_count_models, data_bins, image_location, 'iron_counts')
...
ratio_df.set_index('image', inplace=True)
ratio_df.to_csv('../examine/subset_classify_village_area_image_results.csv')


#Evaluate roof counts
image_data = pd.read_csv('../examine/subset_classify_village_area_image_results.csv')
data_bins = hm.bin_data(image_data, '../data/classify_images/')
count_models = load_models('../models/', 'count_model_')
count_df = apply_models(count_models, data_bins, '../data/classify_images/', 'roof_counts')
count_df.set_index('image', inplace=True)
count_df.to_csv('../examine/subset_classify_village_area_image_results.csv')
...
iron_df.set_index('image', inplace=True)
iron_df.to_csv('../examine/subset_classify_village_area_image_results.csv')
